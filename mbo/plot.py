...
from mbo.metric import goodness_of_fit
...
def parallel_parameters(
    ...
    fig = px.parallel_coordinates(parameters, color="output")
...
def residuals(optimizer: Algorithm, cv_results: dict = None) -> List[go.Figure]:
    ...
    if cv_results is not None:
        ...
        Y = optimizer.problem.data[problem.outputs.names]
        Yp = optimizer.predict(optimizer.problem.data)
