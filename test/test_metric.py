...
def test_adjusted_r2_score():
    ...
    yp = np.array([0.1, 1.1, 2.1, 3, 4, 5])
    r2 = r2_score(y, yp)

    # R²_adj = 1 - (1 - R²) * (n_samples - 1) / (n_samples - n_features - 1)
    assert np.isclose(adjusted_r2_score(y, yp, n_features=1), 1 - (1 - r2) * 5 / 4)
    assert np.isclose(adjusted_r2_score(y, yp, n_features=4), 1 - (1 - r2) * 5 / 1)
    assert np.isneginf(adjusted_r2_score(y, yp, n_features=5))
    assert np.isneginf(adjusted_r2_score(y, yp, n_features=100000))
